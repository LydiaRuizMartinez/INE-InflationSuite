...
class INEExtractor:
    ...
    def _build_date_params(self, start_date: str, end_date: str) -> Dict:
        """
        Build a robust set of date/format parameters covering common INE API variants.

        Many INE/WSTempus endpoints use different names for date and format parameters.
        To improve compatibility we send multiple synonyms; servers will ignore unknown keys.
        ...
        Returns:
            Dict with multiple synonymous parameters
        """
        return {
    ...
    def _convert_fecha_column(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        try:
            ...
            # Strategy 3: Handle string dates with timezone awareness
            else:
                df['fecha'] = pd.to_datetime(df['fecha'], errors='coerce', utc=True)
                # If timezone-aware, drop tz for consistency
                try:
                    if pd.api.types.is_datetime64tz_dtype(df['fecha']):
                        df['fecha'] = df['fecha'].dt.tz_convert(None)
                except Exception:
                    pass
                self.logger.debug("Converted fecha from datetime strings (tz-aware tolerant)")
            
            # Format to standard string format
            # Only format if fecha is datetimelike
            date_format = self.config['output']['date_format']
