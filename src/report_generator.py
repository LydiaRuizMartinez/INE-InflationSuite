...
class ReportGenerator:
    ...
    def _create_time_series_plot(self, data: pd.DataFrame, predictions: pd.DataFrame,
        ...
        def _to_naive_datetime(series: pd.Series) -> pd.DatetimeIndex:
            dt = pd.to_datetime(series, errors='coerce', utc=True)
            # convert tz-aware to naive for consistent plotting
            try:
        ...
        if isinstance(data.index, pd.DatetimeIndex):
            ...
            hist_dates = data.index.tz_convert(None) if getattr(data.index, 'tz', None) is not None else data.index
        elif 'fecha' in data.columns:
            hist_dates = _to_naive_datetime(data['fecha'])
        else:
        ...
        if 'fecha' in predictions.columns:
            pred_dates = _to_naive_datetime(predictions['fecha'])
        else:
...
Máx: {np.max(pred_values):.2f}%"""
    ...
    def _create_confidence_intervals_plot(self, predictions: pd.DataFrame, 
        ...
        if 'fecha' in predictions.columns:
            dates = pd.to_datetime(predictions['fecha'], errors='coerce', utc=True)
            try:
    ...
    def _create_historical_analysis_plot(self, data: pd.DataFrame, output_dir: Path) -> str:
        ...
        # Build aligned DataFrame with dates and inflation values
        def _to_naive_datetime(series: pd.Series) -> pd.DatetimeIndex:
            dt = pd.to_datetime(series, errors='coerce', utc=True)
            try:
                dt = dt.tz_convert(None)
        ...
        if isinstance(data.index, pd.DatetimeIndex):
            dt = data.index.tz_convert(None) if getattr(data.index, 'tz', None) is not None else data.index
        elif 'fecha' in data.columns:
            dt = _to_naive_datetime(data['fecha'])
        else:
            dt = pd.date_range(start='2010-01-01', periods=len(data), freq='M')

        df_plot = pd.DataFrame({
            'dt': dt,
            'inflation': data[inflation_col]
        })
        # Drop rows without date or inflation
        df_plot = df_plot.dropna(subset=['dt', 'inflation'])
        df_plot = df_plot.sort_values('dt')

        # Plot 1: Time series
        axes[0, 0].plot(df_plot['dt'], df_plot['inflation'], linewidth=1.5, color='#2E86AB')
        ...
        # Plot 2: Distribution
        axes[0, 1].hist(df_plot['inflation'], bins=20, alpha=0.7, color='#A23B72', edgecolor='black')
        ...
        # Add statistics
        mean_inf = float(np.mean(df_plot['inflation'])) if len(df_plot) else 0.0
        ...
        if len(df_plot) >= 12:
            monthly_avg = df_plot.groupby(df_plot['dt'].dt.month)['inflation'].mean()
        ...
        if len(df_plot) >= 12:
            rolling_mean = df_plot['inflation'].rolling(window=12).mean()
            rolling_std = df_plot['inflation'].rolling(window=12).std()
            axes[1, 1].plot(df_plot['dt'], df_plot['inflation'], alpha=0.3, color='gray', label='Datos')
            axes[1, 1].plot(df_plot['dt'], rolling_mean, linewidth=2, color='#2E86AB', 
                           label='Media Móvil (12m)')
            axes[1, 1].fill_between(df_plot['dt'], 
    ...
    def create_technical_report(self, analysis: Dict[str, Any], 
        ...
        try:
            ...
            story.append(Paragraph(f"Dirección y desarrollo del proyecto: María González Gómez y Lydia Ruiz Martínez", styles['Normal']))
