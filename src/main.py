...
class InflationPredictionPipeline:
    ...
    def step_4_model_training(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if target_columns:
                ...
                # Keep only numeric features to avoid strings like 'Definitivo'
                X = engineered_data[feature_columns].select_dtypes(include=[float, int, np.number])
    ...
    def _save_pipeline_state(self) -> None:
        ...
        try:
            ...
            # Helper to convert complex objects to JSON-safe structures
            def _json_safe(obj):
                try:
                    import pandas as pd
                    import numpy as np
            ...
            # Sanitize results and metrics to be JSON-safe
            if 'results' in serializable_state:
                serializable_state['results'] = _json_safe(serializable_state['results'])
            
            # Add performance metrics (sanitized)
            serializable_state['performance_metrics'] = _json_safe(self.performance_metrics)
