...
class ModelTrainer:
    ...
    def _prepare_ml_data(self, X: pd.DataFrame, y: pd.Series) -> Tuple[pd.DataFrame, pd.Series]:
        ...
        # Ensure numeric dtype and sanitize infinities
        # Coerce non-numeric to NaN (should already be numeric, but safe)
        for col in X_aligned.columns:
            if not np.issubdtype(X_aligned[col].dtype, np.number):
                X_aligned[col] = pd.to_numeric(X_aligned[col], errors='coerce')
        # Replace +/-inf with NaN
        X_aligned.replace([np.inf, -np.inf], np.nan, inplace=True)
        try:
            y_aligned = pd.to_numeric(y_aligned, errors='coerce')
        except Exception:
            pass
        y_aligned.replace([np.inf, -np.inf], np.nan, inplace=True)

        # Handle missing values
        X_clean = X_aligned.fillna(method='ffill').fillna(method='bfill')
        # Any remaining NaN set to 0 for features
        X_clean = X_clean.fillna(0)
        y_clean = y_aligned.fillna(method='ffill').fillna(method='bfill')

        # Remove any remaining NaN or non-finite rows
        finite_mask = np.isfinite(X_clean).all(axis=1) & np.isfinite(y_clean)
        valid_mask = finite_mask & ~(X_clean.isnull().any(axis=1) | y_clean.isnull())
