...
class DataProcessor:
    ...
    def normalize_dates(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        # Common date column names to look for (case-insensitive)
        date_columns = ['fecha', 'date', 'periodo', 'period', 't3_periodo']
        ...
        for col in normalized_data.columns:
            ...
            if any(date_col in col.lower() for date_col in date_columns):
                try:
                    ...
                    # First attempt: tolerant parse, keep timezone info
                    ...
                    # Drop timezone to keep everything naive (matplotlib/pandas friendly)
                    if pd.api.types.is_datetime64tz_dtype(normalized_data[col]):
                        normalized_data[col] = normalized_data[col].dt.tz_convert(None)
                    ...
                    if failed_conversions > 0:
                        ...
                        # Second attempt: handle INE patterns like 'YYYYMmm' or 'YYYY-mm' explicitly
                        try:
                            import re
                            def _parse_custom(v):
                                if pd.isna(v):
                                ...
                                # YYYY-Qn or YYYYQn -> first month of quarter
                                m = re.match(r'^(\d{4})[- ]?Q(\d)$', s, re.IGNORECASE)
                                if m:
                                    y, q = int(m.group(1)), int(m.group(2))
                                    mo = {1:1,2:4,3:7,4:10}[q]
                                    return pd.Timestamp(year=y, month=mo, day=1)
                                # ISO with Z timezone
                                try:
                                    ...
                                    if pd.isna(ts):
                                        return pd.NaT
                                    return ts.tz_convert(None) if hasattr(ts, 'tz_convert') else ts
                                except Exception:
                                    return pd.NaT
                            normalized_data[col] = normalized_data[col].apply(_parse_custom)
                            remaining = normalized_data[col].isnull().sum()
                            if remaining < failed_conversions:
                                self.logger.info(f"Recovered {failed_conversions-remaining} dates in '{col}' using custom parser")
                        except Exception as e2:
                            self.logger.debug(f"Custom date parsing fallback failed for '{col}': {e2}")
                    else:
            ...
            # Also check if column contains date-like strings
                ...
                if len(sample_values) > 0:
                    ...
                    try:
                        pd.to_datetime(sample_values.iloc[0], utc=True)
